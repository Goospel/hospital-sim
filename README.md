...
한 주는 **7일의 진료 + 주말의 응급 한 번**이다. 병원을 세우고(채용), 매일 밀려드는 환자를 한정된 자리로 받고, 주말엔 급성 심근경색(STEMI) 응급을 맞는다. 한 주가 끝나면 세계가 바뀌고(법·수가·여론), 계속할지 끝낼지는 당신이 정한다.

- **응급실 뺑뺑이** — 우리 병원이 못 받으면 골든타임 180초 안에 6개 병원에 전화를 돌린다. 병상이 있어도 "배후진료 불가"로 거절당한다. 받아줄 수 있는 곳은 6곳 중 단 하나.
- **필수의료 붕괴** — 결말의 '병원 장부'가 왜 그 벽이 있었는지 보여준다: 병원은 미용·검진으로 흑자인데, 환자가 필요한 순환기 채용은 0.

게임은 결말에서 주제를 **설명하지 않는다**. 숫자(영수증·장부·신문)만 병치하고 플레이어가 스스로 잇는다. (사실 근거·각색 고지: [STEMI 팩트시트](docs/research/stemi-factsheet.md))

## 플레이 방법

1. **개원** — 미용·검진(돈 되는 과)과 필수과(순환기 등) 채용을 정해 병원을 연다.
2. **7일 진료** — 매일 밀려드는 환자를 한정된 자리로 받거나 돌려보낸다. 하루 끝마다 손익이 쌓이고, 이튿날 아침 신문에 어제 돌려보낸 사람들이 실린다.
3. **응급** — 주말 STEMI. 우리 병원이 받거나, 못 받으면 **전원 콜**을 돌린다. 거절되면 사유 태그가 붙는다(병상 없음 / 응급실 당직 없음 / 과밀 / 배후진료 불가). 자유 텍스트로 **매달려도** 판정은 안 바뀐다 — 구조의 벽을 손끝으로 겪는 부분.
4. **주간 결산** — 이번 주·누적 손익과 응급 생존을 보고 [다음 주] 또는 [종료]를 고른다. [다음 주]는 바뀐 세계 위에서 다시 시작된다.
5. **결말** — 종료하면 **누적 결산 장부 + 이번 주 신문**(영수증·명부)이 병치된다.
...
| [docs/submission/ai-usage-doc.md](docs/submission/ai-usage-doc.md) | AI 활용 기술 문서 — 'AI 디렉팅'으로 게임을 만든 과정(제출물 ④) |
