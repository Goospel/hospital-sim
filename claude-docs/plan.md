...
- 🔜 **림월드형 2주차 — 「림월드가 되는 주」** (PR A~D 분해 · 우선순위 결정 2026-07-27: 시간 부족 시 PR C 절단·PR D(AI 스토리텔러) 사수, LLM은 폴백 우선):
  ...
  - ✅ **PR C — 욕구·우선순위·사직** (2026-07-28 완료 · 절단 후보였으나 완주): **욕구 2종** — 휴식(LOUNGE 의자로 걸어가 60분 · 피로 −15 · 밤 회복 20보다 반드시 약함이 계약)·허기(CAFETERIA · 300분 뒤 배고픔 · 식사 20분 · 굶은 채 시작한 작업 **1.15배** 감속, 식사로만 풀림). 두 갈래가 `BREAKS` 표 한 기계를 공유하고 **표 순서가 곧 우선순위**(휴식 먼저 — 되돌릴 수 없는 쪽이 급하다). ⏸ **스트레스는 절단**: 행동 규칙이 피로와 동형(임계→행동 중단→회복)이라 셋째 축이 새 결정을 안 낳는다 — 개성 상호작용이 붙어야 뜻이 생기므로 **PR D(스토리텔러) 재료**로 넘긴다. **우선순위 토글 0~3**(진료·응급·휴식 세 축 · 기본 2 · 화면 순환 2→3→1→0). 응급 하드락 `NO_SPECIALIST` 정의가 "그 과 의사 ≥1"에서 **"응급을 켠 그 과 의사 ≥1"**로 확장됐다(토글이 실제로 벽을 세운다). **사직·전국 풀** — 포화(피로 100)로 마감한 날이 **리셋 없이** 쌓여 4일에 닿으면 주간 결산에서 통지하고 **다음 주에 집행**, 전국 풀(미용 8·내과 5·외과 3·순환기 2)로 **돌아오지 않는다**. 리팩터 둘: `spots.ts` **leaf 추출**(needs ⇄ patientFlow 값 임포트 순환 제거 — [T-093](troubleshooting/T-093.md))·`workDurationMin` 통합(피로×허기 감속의 단일 출처). **958 vitest**·tsc 0·eslint 0(신규 `no-restricted-imports` 포함)·build+Pages export·브라우저 실측(채용→건설→나흘 포화→휴게실·식당 관측→토글 4단 순환→1주차 결산 사직 통지→다음 주 소멸·잔여 미반환). 함정 4건 등재([T-091](troubleshooting/T-091.md)~[T-094](troubleshooting/T-094.md)) + [T-086](troubleshooting/T-086.md) 보강. 상세 changeLog·[플랜](../docs/superpowers/plans/2026-07-28-rimworld-week2-needs-resignation.md)
    ...
    - ⚠️ **이월(밸런스) — 최종 브랜치 리뷰 프로브가 재실측 예정**: ⓐ 순환기 1인 + 응급 병원이 **1주차 결산에서** 사직한다(응급 강도 2.0·90분이 하루 상한을 혼자 채운다 — 기존 게임의 "2주차 결산"보다 빠르고, 휴식이 생겼는데도 그렇다). 대응 창이 ~3일뿐이라 **인사 패널의 포화 N/4일 표시**로 코어 튜닝 없이 넓혀 뒀다 ⓑ **rest 토글이 응급 피로에 무력** — 토글은 외래 배정에서만 빼 주는데 피로의 주범이 응급이라, 태업 조합(rest 3·휴게실 없음)도 사직을 **못 늦췄다**(바뀐 건 외래 17→7건뿐) ⓒ **튜닝 손잡이 소속** — `EMERGENCY_INTENSITY`·`durationMin`은 `src/sim` 자유값이지만 `RESIGN_SATURATED_DAYS`·`FATIGUE_*`는 `src/game`과 **공유**라 한쪽만 돌리면 기존 게임이 함께 움직인다(갈라야 하면 `DEFAULT_PRIORITY` 패턴 — 같은 상수를 쓰는 것이 의도된 등식임을 주석으로 못박고 필요할 때 분기)
  ...
  - ⬜ **PR D — AI 스토리텔러 + 스왑·제출물** (사수): 결정론 이벤트 엔진→LLM 디렉터(무키 폴백 — 실호출은 결제 확인 후 비파괴 가산) ⬜ 랜딩 스왑 ⬜ 영상·PDF. ⚠️ PR A 이월: jsdom 도입해 UI 배선(effectiveSpeed·결산 1회 가드) 잠그기 · 금고 표기 단위 혼재(HUD 만원 vs 오버레이 억 접기 — `formatManwon` 접는 경계 정책)
  - ⬜ **이월 항목**(1주차 리뷰가 남김): 스페이스바 일시정지·9~19시 운영 시간대(스펙 §1 — 1주차 계획에 미배정, 버튼·0..480분만 구현) · 의사 재홈 없음(고립 방에 갇히면 영구) · 철거 없음 · `/sim` 진입 링크 없음 · RECEPTION 수납 흐름(`PAYING`/`GONE` 예약) · 표시층 상호작용 테스트 0건 · 터치 가로 스크롤 불가 · 대각선 보간이 벽 모서리를 시각적으로 스침 · `Skills-used` 트레일러가 Task 1~2 커밋에서 과다보고(템플릿 오류 — Task 3부터 `none`으로 정정, 집계 시 참고).
