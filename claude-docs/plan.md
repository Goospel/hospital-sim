...
- 🔜 **림월드형 2주차 — 「림월드가 되는 주」** (PR A~D 분해 · 우선순위 결정 2026-07-27: 시간 부족 시 PR C 절단·PR D(AI 스토리텔러) 사수, LLM은 폴백 우선):
  ...
  - ⬜ **PR D — AI 스토리텔러 + 스왑·제출물** (사수): 결정론 이벤트 엔진→LLM 디렉터(무키 폴백 — 실호출은 결제 확인 후 비파괴 가산) ⬜ 랜딩 스왑 ⬜ 영상·PDF. ⚠️ 이월: jsdom 도입해 UI 배선(effectiveSpeed·결산 1회 가드) 잠그기 · 금고 표기 단위 혼재(HUD 만원 vs 오버레이 억 접기 — `formatManwon` 접는 경계 정책) · 스페이스바 일시정지 **이월 항목**(1주차 리뷰가 남김): 스페이스바 일시정지·9~19시 운영 시간대(스펙 §1 — 1주차 계획에 미배정, 버튼·0..480분만 구현) · 의사 재홈 없음(고립 방에 갇히면 영구) · 철거 없음 · `/sim` 진입 링크 없음 · RECEPTION 수납 흐름(`PAYING`/`GONE` 예약) · 표시층 상호작용 테스트 0건 · 터치 가로 스크롤 불가 · 대각선 보간이 벽 모서리를 시각적으로 스침 · `Skills-used` 트레일러가 Task 1~2 커밋에서 과다보고(템플릿 오류 — Task 3부터 `none`으로 정정, 집계 시 참고).
