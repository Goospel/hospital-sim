...
- 🔜 **F1b. 24시간 당직 커버리지 — 지뢰 4의 해답** (F1에서 분리, 사용자 결정)
  - **문제**: 순환기 n≥2가 **강지배**된다 — 부문 손익은 `−12 × n`인데 2번째 의사가 사는 게 0이다(`backupCare`는 **이진값**이라 1명이든 5명이든 게이트 통과가 같고, `beds`·STEMI 14통이 고정이라 처리량도 안 는다)
  - 🔴 **기각된 답 — "위저드 상한 1"**: 코드가 이진값이니 게임도 이진으로 맞추자는 것이었다. **코드의 한계를 현실의 진실로 착각**한 것이고(T-039 단위 혼용과 같은 계열), 사용자가 정확히 그렇게 기각했다 — *"현실에선 순환기 의사가 2명 배치될 수 있잖아. 한 명으로 강제하면 게임이 저렴해 보이고 만들기 쉽게 하려고 타협한 느낌이 든다."*
  - ✅ **채택 — 2번째 의사가 사는 건 처리량이 아니라 `시간대 커버리지`다.** 근거가 이미 리서치에 있었다: STEMI는 **"24시간 중재 순환기내과"**를 갖춘 PCI 병원이라야 재관류가 된다([medical-system-grounding.md](../docs/research/medical-system-grounding.md):20) · 거절하는 당직의의 정당한 제약은 **"당직 1명이 이미 3명 동시 진료"**(:66) · 수용곤란 고지의 **인력부족 사유가 1년 새 2.3배**([stemi-factsheet.md](../docs/research/stemi-factsheet.md):19) · I4의 근거 자체가 **"24시간 대기 고정비"**
  - ⬜ 콜에 **주간/야간** 부여 — `DAY_PLANS` 위치에서 파생(**RNG 0**, 결정론 유지)
  - ⬜ **순환기 1명 = 주간만** 배후 성립 → 야간 STEMI는 `NO_BACKUP_CARE`. **2명 = 24시간**
  - ⬜ 그러면 딜레마가 **"순환기를 뽑았다" → "밤에도 받게 두 명을 뽑았다(60억 = 예산의 60%)"**로 날카로워지고, **밤에 거절한 STEMI가 곧 F3 신문의 재료**가 된다. 게임의 거절 사유가 현실의 최대 거절 사유(인력부족)와 처음으로 일치
  - ⚠️ 순환기 상한은 그때 **2(=당직 로테이션 최소 인원)**로 좁힐지 재검토 — 지금은 `FIXED_BEDS`(3) 파생이라 3명째가 여전히 강지배다
