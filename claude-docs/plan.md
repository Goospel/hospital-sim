...
- 🔜 **UI 통합(Part 2)** — `SetupWizard`·`ReceivingPhase`·`SessionClient`·에필로그 배선 + 브라우저 검증. 진입=세션이 THE 게임(위저드-first, 기존 STEMI는 2막 TRANSFER로 흡수)·통일 시각(단일 어두운 zinc 시스템 + 초록→빨강 액센트 아크, 밝은/어두운 화면 전환 없음). 설계: [docs/superpowers/specs/2026-07-17-management-expansion-ui-design.md](superpowers/specs/2026-07-17-management-expansion-ui-design.md). (로직 코어 Minor 4건: 음수값 방어→Part2 해소 / essentialHires 다필수과·도달불가 분기 커버·STEMI_SPECIALTY export→v2·별건 보류)
