...
- ⬜ **제출 요강/폼 전문 정독** → **미확인 항목 목록은 [requirements.md 「❓ 아직 확인 못 한 것」](../docs/submission/requirements.md)이 단일 출처다**(여기 다시 적지 않는다). 그중 **1번 비고 `GitHub Pages`가 예시인지 지정인지**가 최우선 — 지정이면 정적 export 작업이 생긴다. 확인한 항목은 그 문서에서 체크한다
...
- 🔜 **③④ PDF 최종화** (P7, 8/3~8/7) — 제출 패키지 골격은 섰다([docs/submission/final/](../docs/submission/final/) · `npm run pdf`로 재생성, 원본은 md). **남은 것만**: ⓐ 저장소 공개 URL 확정치 기입(현재 `⏳ [최종 제출 URL 기입]` 2곳) ⓑ 스크린샷을 **배포본** 기준으로 재캡처(현재는 로컬 dev 빌드) ⓒ ④§6-4 런타임 LLM 프롬프트↔응답·폴백 강등 로그(U2 이후) ⓓ 제출 요강 재확인(마감 시각·타임존·빌드 제출 형식)
...
- ⬜ LLM 프록시(Next API 라우트, 서버 키·타입드 로더 검증) + 대사콜 풀버전(경영/협상 대사를 폴백 위에 얹음)
