...
- 🔜 **림월드형 2주차 — 「림월드가 되는 주」** (PR A~D 분해 · 우선순위 결정 2026-07-27: 시간 부족 시 PR C 절단·PR D(AI 스토리텔러) 사수, LLM은 폴백 우선):
  ...
  - 🔜 **PR B — 과·경제·피로 이식**: EXAM 과 지정 + 인게임 채용 최소판 ⬜ 과별 수가/고정비(`CALL_ECONOMICS` 계승 — PR A의 `WEEKLY_DOCTOR_COST_MANWON` 상수를 과별로 정밀화) ⬜ 응급 환자(배후과 없으면 못 받는다 — 메시지의 심장) ⬜ 피로 표준강도·감속 이식
