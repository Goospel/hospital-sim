...
### 🔜 다음
- 🔜 **채용 데스크 v2 — 빈 병원 맵 허브(무스크롤 1화면)** (2026-07-24 설계 확정, brainstorming): 사용자 요구 ①데스크톱 무스크롤 1화면 ②웹이 아니라 게임답게. v1의 세로 아코디언을 버리고 **개원 전 빈 병원 맵**을 화면 전체로 — 현판(이름 입력 통합·NAME 화면 삭제) + 맵(방 클릭→지원서 4장 중앙 오버레이·도장 즉시 그 방에 아바타) + 하단 바(장부·자격·개원). 신규는 `deriveSetupScene`(순수 파생)·`HospitalMap.onRoomClick`(옵셔널)뿐, 판정·경제·채용 로직 0줄. 설계 [spec](../docs/superpowers/specs/2026-07-24-hiring-desk-map-ui-design.md)
