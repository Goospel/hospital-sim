...
### 🔜 다음 — 응급 재설계 B (다양화)
- 🔜 **응급 CallKind 다양화**: 분만(산부)·뇌출혈(신경외과)·중증외상(외과)을 받는 콜로 신설 — 각 배후 없으면 NO_BACKUP_CARE 하드락(adjudicate 제네릭 재사용). DAY_PLANS 재구성 + news 종류별 일반화. 이로써 재정중립 산부 −16 상쇄가 장부에서 체감된다. A(spec §7) 위에 얹는다
...
### 본선 48시간 확장 (신청 이후 보류)
- ⏸ **F4 남은 축 = 지정 티어** — 권역심뇌혈관센터 "심장 수술 전문의"(별표 2) · 권역외상센터 전담 1명 겸직 불가 · 목포중앙병원 2022 지정 철회(전남 4년 공백). 사실이지만 **콜이 아니라 병원 등급**이고 시간 척도가 7일보다 큼(지정·평가·철회 연 단위) → 접수 화면이 아니라 위저드/결말의 자리. 재량 예외도 있어 **하드 게이트가 아니라 압력**. 자산: §4 가산금 배분 규정 조사 유효([thoracic-surgery-role.md](../docs/research/thoracic-surgery-role.md))
- ✅ **재정중립 제로섬 — 최소 슬라이스 착수** (2026-07-19): 개원 이벤트(`OPENING_EVENT`)를 순환기 +6 / 산부 −6 재정중립 정책수가 패키지로 — 급여 풀 profit 합=0, 미용(비급여) 무풍지대가 개원 위저드 숫자에서 드러남. "필수과 수가가 안 오르는 이유가 악의가 아니라 산수"를 값으로. 설계: [2026-07-19-fiscal-neutral-opening-event-design.md](../docs/superpowers/specs/2026-07-19-fiscal-neutral-opening-event-design.md), 근거: [fee-schedule-and-subsidies.md](../docs/research/fee-schedule-and-subsidies.md) §6. ⏸ **남은 것(본선)**: 산부 콜로 장부 체감 · 매주 재구성이 economics에 물리게(2주차+ 실효) · hireCost 손잡이 복원 — 정책 레이어 통째 신설은 여전히 마감 후 후보로 가장 강함
