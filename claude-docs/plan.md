...
- 🔜 **F1. 순환기 고정비 재정의 + 콜당 수가/원가 내역** — 나머지 셋의 전제(절단 불가)
  - ⬜ 순환기 부문 = "적자과" → **24시간 심장중재팀 대기 고정비**로 재정의(숫자 유지·의미 변경)
  - ⬜ 콜 델타 부호를 **행위 유형 표 하나에서만** 뽑는다(단위 혼용 금지): 일반 응급 **+→−**(기본진료 50.5%·응급수가 45%) / **STEMI는 음수 유지**하되 크기를 줄임(수술·처치 **84.9%** — 원가 미만이되 기본진료보다 덜 밑진다). **과 단위 117%·103%는 입력이 아니라 F2에서 창발한다**
  - ⬜ 콜당 두 줄 노출: **"진료비 (병원 책정)"**(비급여) vs **"수가 (정부 고시)"**(급여) + 원가. 해석 카피 0
  - ⬜ **결정 필요(지뢰 4)**: 순환기 n≥2가 **강지배**된다 — 부문 손익은 `−N·n`인데 STEMI 콜 14통·자리 3이 고정이라 2번째 의사는 처리량을 1통도 못 늘린다. 위저드 상한 1 / n으로 자리 완화 / 문서화만 중 택1. ⚠️ **레포 대표 픽스처가 바로 n=2**(`{AESTHETICS:1, CARDIOLOGY:2}`)
  - ⚠️ 부호가 바뀐다 → **브라우저 7일 완주 2회 필수**. PR #35의 −525억이 vitest 372 green을 뚫고 나온 전력
...
- ⬜ **별건 (F와 독립 · 적대 검증 발견)** — **과별 인원 상한이 없다**: `withinBudget`이 `hiringCost ≤ 100`만 보고 `adjustDoctors`·`isSetupReady`·`SetupWizard.tsx` 어디에도 과별 상한이 없어 **미용 10명(정확히 100억)이 합법 셋업**이고 부문 손익 `70 × 10 = **+700억/주**`다. 예산 100억 병원이 주당 예산 7배를 버는 장부. 스펙 I8(`|순이익| ≤ 4 × 예산`)이 이걸 red로 잡는다
