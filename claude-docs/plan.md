...
- 🔜 **③④ PDF 최종화** (P7, 8/3~8/7) — 제출 패키지 골격은 섰다([docs/submission/final/](../docs/submission/final/) · `npm run pdf`로 재생성, 원본은 md). **남은 것만**: ~~ⓐ 저장소 공개 URL 확정치 기입~~ (완료 2026-07-22 — 플레이는 Pages, 소스는 저장소 URL로 3종 확정) ⓑ 스크린샷을 **배포본** 기준으로 재캡처(현재는 로컬 dev 빌드) ⓒ ④§6-4 런타임 LLM 프롬프트↔응답·폴백 강등 로그(U2 이후) ⓓ 제출 요강 재확인(마감 시각·타임존·빌드 제출 형식)
