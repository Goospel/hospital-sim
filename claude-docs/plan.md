...
- ✅ 작업추적 3종(plan/changeLog/troubleshooting) 생성 — `claude-docs/` 배치(PR #22). *learning-notes는 빈 골격으로 만들었다가 30여 커밋 동안 미사용이라 PR #22에서 삭제 — 이 프로젝트는 지식이 문서가 아니라 코드로 굳는다.*
...
- ✅ **Karpathy LLM Wiki 패턴 대조**(46에이전트: 조사 5 → 매핑 → 3렌즈 적대검증 → 구멍분석) → **판정: 우리는 LLM Wiki가 아니다**(9요소 중 strong 0 / partial 1 / stretch 6 / 부재 2). 트리거가 source-push가 아니라 **feature-pull**이라 기존 페이지를 재검토할 계기가 원리적으로 없음 → 지식이 문서가 아니라 **코드로 굳는다**. 근거: learning-notes 30커밋 0줄→삭제, PR #18이 29에이전트 돌리고 기존 리서치 갱신 0건.
- ✅ **lint 실행**(120에이전트: 5축 스윕 → 3렌즈 검증 → 38건 중 **16건 기각**) → `game-concept.md` 13건 교정. 부트스트랩(`93f18ea`) 이후 34커밋 무갱신이라 **게임 1막(설립 위저드·콜 트리아지) 전체를 문서가 모르고 있었음**. 숨은 제약 2종→4종, 코어 루프→실제 6페이즈, 미구현(반사실·지도·문진·LLM저지)에 ⏸ 표기, 톤 가드레일은 본문 복제 대신 리서치 포인터(사본은 확정적으로 drift), 깨진 앵커 2건. 앵커·상대링크 실측 0 broken.
