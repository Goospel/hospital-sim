...
### F 딜레마 — 남은 갈래
- 🔜 **순환기 상한을 `ROUND_THE_CLOCK_MIN_DOCTORS`(2)로 좁힐지 재검토** — 지금은 `FIXED_BEDS`(3) 파생이라 **3명째가 강지배**(24시간을 두 번 사지 못함). F3 완료됐으니 이제 판단 시점 (was ⚠️ — F3 뒤 재검토 대기였음)
...
- ⏸ **재정중립 제로섬** — 상대가치 개편은 총점 고정 전제 → 한 과를 올리려면 다른 과를 내린다("재정 돌려막기"). **필수과 수가가 안 오르는 이유가 악의가 아니라 산수** = 이 게임 주제 그 자체. 정책 레이어를 통째 신설해야 해 마감 대비 위험 큼. **본선 확장 후보로 가장 강함.** 근거: [fee-schedule-and-subsidies.md](../docs/research/fee-schedule-and-subsidies.md) §6
