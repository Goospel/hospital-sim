...
**밸런스 관측(이월 · 최종 리뷰 프로브가 재실측)** — 순환기 1인 + 응급 병원이 **1주차 결산에서** 사직한다. 휴식이 생겼는데도 기존 게임(2주차 결산)보다 **빠르다** — 응급(강도 2.0·90분)이 하루 상한을 혼자 채우기 때문이다. 대응 창이 ~3일뿐이라 코어 튜닝 대신 **인사 패널의 포화 N/4일 표시**로 넓혀 뒀다. rest 토글은 이 곡선을 **못 늦춘다**(외래 배정에서만 빼 주는데 피로의 주범이 응급이다 — 태업 조합에서 바뀐 건 외래 17→7건뿐).
