...
- [T-157](troubleshooting/T-157.md) · 전체 스위트에서만 5~6건이 무작위로 죽고 단독 실행은 통과 — 1순위 용의자는 전역 오염이지만 실패 목록이 「그 파일의 최슬로우 테스트들」과 일치하면 범인은 오염이 아니라 타임아웃이다. fork 병렬 CPU 경합이 기본 5초를 넘겼다
- [T-156](troubleshooting/T-156.md) · 영상 하네스의 12주 빨리감기가 40분 상한에서 11주 6일로 끊겼다 — __ffwd는 프레임당 게임분을 넘기므로 소요 시간은 게임 안 날수가 아니라 실제 렌더 프레임 수다. UI가 무거워지면(팔레트 상주 로스터) 옛 타임아웃이 조용히 부족해진다
...
- [T-142](troubleshooting/T-142.md) · 정적 값을 테스트로 잠가도 CSS 필터(hover:brightness-* 등)가 런타임에 그 값을 변형해 계약을 우회한다 — 필터 결과색은 어떤 테스트에도 안 잡히므로, 필터 방향을 "계약이 단조 보존되는 쪽"(흰 글자 위 배경이면 어둡게)으로 잡아야 한다
