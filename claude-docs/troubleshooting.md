...
- [T-155](troubleshooting/T-155.md) · 전체 스위트에서만 5~6건이 무작위로 죽고 단독 실행은 통과 — 1순위 용의자는 전역 오염이지만 실패 목록이 「그 파일의 최슬로우 테스트들」과 일치하면 범인은 오염이 아니라 타임아웃이다. fork 병렬 CPU 경합이 기본 5초를 넘겼다
