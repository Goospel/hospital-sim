...
> ⚠️ **위 표는 계획 시점의 시작점이고, 고정비·응급 수가는 Task 5에서 재조정됐다**(그 절이 허용한
> "실측으로 조정 가능"의 실행). 최초값으로 돌리면 부호가 통째로 뒤집힌다 — 실측에서 미용은 적자였고
> 순환기는 응급만으로 주 +10,000 흑자였다. **현행 값의 단일 출처는 `src/sim/dept.ts`(수가·고정비·강도)와
> `src/sim/emergency.ts`(응급 수가·소요)**이고, 조정 근거는 그 값별 주석에 있다. 부호는 `week.test.ts`의
> I-B1이 잠근다. 여기 표를 최신값으로 덮어쓰지 않는 이유는 그러면 값이 두 곳에 적혀 한쪽이 조용히 낡기
> 때문이다(이 저장소가 세 번 경고한 이중 기재).
