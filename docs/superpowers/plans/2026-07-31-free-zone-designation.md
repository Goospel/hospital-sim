...
- [x] **Step 3: 브라우저 실측** (코디네이터 수행 — 2026-07-31 · 6항목 전부 통과):
  1. **접수처를 문 없이**: 카운터 위에만 칠했는데도 간호사 셋이 창구에 섰다 — 경고가 *"창구는 지금 서 있습니다 — 샌 진료비는 그 전의 것입니다"*로 바뀌고 「수납 창구를 만듭니다」 ✔. dfc2388의 가구 오탐 수정도 이 배치로 함께 실증(길 경고 안 뜸).
  2. 열린 대기실(벽 없음)에 환자가 와 앉고 진료 2건이 돌았다.
  3. 이어 칠하면 방 수 유지(병합), 가운데 지정 해제로 +1(분할) — aria-label 방 수로 계측.
  4. 벽 두른 병동에 「길이 닿지 않는 영역 1개 — 벽에 문을 내세요」 → 문 내니 해소.
  5. 휴게실 덮어쓰기 성공 + 부분 건너뜀 토스트("이미 같은 용도라 건너뛰었습니다").
  6. 새 문구(「모든 영역에 길이 닿게 합니다」·드래그 힌트·「지정 해제」 버튼·"용도 지정 — 무료") 전부 확인.
