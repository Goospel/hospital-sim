...
검산 결과(전부 통과):
- 램프 방향 `accent > body > seat > leg`, 최소 단차 5.6
...
`preview_start`로 dev 서버를 띄우고(`.claude/launch.json` 참조) 실제로:
1. [건설] → [의자] → 종류 줄이 열리는가
2. 다섯을 각각 골라 놓고 **화면에서 갈리는가**
3. **벤치를 가로로 3칸 이상** 놓아 하나로 이어지는가
4. 환자가 앉았을 때 **그 변종의 좌면**이 하체를 덮는가(소파·벤치에서 특히)
5. 도구를 벽으로 바꿨다 의자로 돌아왔을 때 **고른 변종이 유지되는가**
