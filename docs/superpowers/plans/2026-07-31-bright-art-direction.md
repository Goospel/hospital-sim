...
Expected: FAIL — `$name: 모든 항목이 구간 안에 있다` 8건이 전부 실패하고, 실패 메시지에 `ground(#1a1a26) = 26.xx` 형태로 현재 휘도가 찍힌다. 서열·간격 단언은 아직 통과한다(값을 안 바꿨으므로).
...
Run: `npx vitest run src/components/backdropPalette.test.ts`
Expected: PASS — 구간 8건 + 서열 + 간격 + 상한 전부 통과.

> 간격 단언이 통과하는 이유: 모든 값이 같은 배수로 올라 카테고리 평균 간격도 2.4배가 된다. 이 계획이 배수를 고른 이유가 이것이다.
