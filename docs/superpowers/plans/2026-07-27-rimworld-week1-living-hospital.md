...
동작 정의(1주차 절단): 환자는 입구(그리드 아래 중앙 `ENTRANCE = {x: 24, y: 31}`)에서 스폰 → 대기실 빈 의자로 걸음(`WAITING`) → 빈 진료실+유휴 의사가 있으면 배정돼 진료실로(`TO_EXAM`→`IN_EXAM`, `EXAM_DURATION_MIN = 20`분) → 끝나면 수납 없이 바로 퇴장(`LEAVING` — ⚠️ 초안의 "RECEPTION 경유" 산문은 전이표와 모순이라 삭제(2026-07-27 최종 리뷰). 수납 흐름(`PAYING`)은 2주차 예약) → 입구 타일 도달 시 `GONE`(배열에서 제거). 진료 종료 시 `treasuryManwon += EXAM_REVENUE_MANWON(30)`. 대기 `PATIENCE_MIN = 90`분 초과 시 `LEFT_WAITING`으로 퇴장(수익 0, `world.leftCount` 증가). 도착 생성은 `seededUnit`(기존 `src/game/daysim.ts` 재사용)으로 분당 확률 판정 — 주간(0..480분)에 평균 8분당 1명.
