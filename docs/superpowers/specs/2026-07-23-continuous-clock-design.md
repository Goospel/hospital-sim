...
```
flowing === true   →  흐름 패널 (상태 문구 + 건너뛰기 버튼)
flowing === false  →  콜 카드 (지금과 동일)
```

흐름 중엔 결정할 게 없으므로 카드가 없다. 그 자리엔 상태 문구와 건너뛰기 버튼만
남는 흐름 패널을 놓는다.

> **정정(2026-07-23, 플레이 중 발견).** 이 절은 원래 문구를 **마감 대기 문구 하나**로
> 적었다. 그건 `receiving.done`일 때만 참이라, 정작 대부분의 시간인 **콜과 콜 사이에는
> 패널에 버튼만 남았다** — 아래 §7이 잡아둔 `min-h`가 그 공백을 304px 빈 상자로 키웠다.
> 스펙이 조건부 문구를 패널의 **유일한** 내용물로 정하면서 그 조건이 거짓인 경우를
> 안 적은 것이라, 구현·리뷰 어디서도 이탈이 아니었다. 상태 문구는 **항상** 뜬다:
> `done`이면 「오늘 콜은 모두 처리했습니다 · 마지막 진료가 끝나기를 기다립니다」,
> 아니면 「다음 콜을 기다립니다」. 둘 다 지금 무슨 일이 일어나는지만 말한다(해석 0).

**직전 콜 결과**(`prevLine`, 이미 계산되는 값)는 흐름 패널
