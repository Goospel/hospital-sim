...
| 침대 배정이 불안정하다(0번부터 재포장) | `deriveMapScene`은 매 호출마다 진료 중인 의사를 앞에서부터 다시 채운다. 0번 침대 환자가 퇴원하면 남은 환자가 0번으로 밀리고 마지막 침대가 비는 것처럼 보인다 — 인원수는 항상 맞지만 "그 침대"의 연속성은 없다. 안정적 배정은 상태(어느 환자가 어느 침대인지 기억)가 필요한데, 그건 이 설계가 의도적으로 금지한 게임 상태 추가다(§3) | 침대별 점유 이력을 별도 상태로 승격 — 0 침습 전제를 깨야 하므로 본선 확장 |
