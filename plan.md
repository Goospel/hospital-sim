...
### U4. 인과 디브리핑 (결정론 버전) (7/16) — ① ✅
- ✅ 로그 → 구조 변수 3개 **결정론 산출**(`debrief.ts` 순수 함수, TDD 10 tests): 지배 병목=배후진료 불가 / 병상 있었는데 거절한 곳 수 / 표방 N곳 대비 실제 수용 가능 1곳 — 병상 아니라 배후진료 중심(R3.5 경고 반영)
- ✅ 반사실 1쌍 분할 화면(당신을 바꿨다면=불변 / 구조를 바꿨다면=배후진료 +1이면 첫 콜 수용) + 결말별 착지 카피 + 톤 가드레일 + '다시 한 판' — GameClient 배선, 브라우저 검증(DIED/ACCEPTED), 48 green·next build 통과
