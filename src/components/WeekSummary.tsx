...
/**
 * 주간 결산 — 한 주(7일 + 응급의 밤)를 닫고 사용자가 계속할지 끝낼지 고르는 갈림길.
 *
 * 게임이 한 주로 끝나지 않는다는 걸 만드는 화면이다: [다음 주]로 세계가 다시 바뀌며 이어지고,
 * [종료]로 사용자가 끝을 정한다. 해석 0(메모 game-show-dont-tell): 이번 주/누적 숫자와 생존 사실만
  ...
  return (
    <main className="mx-auto flex min-h-full w-full max-w-md flex-1 flex-col justify-center gap-8 px-5 py-10 text-zinc-100 bg-zinc-950">
      <header className="flex flex-col items-center gap-1 text-center">
        ...
        <span className={`text-xs uppercase tracking-[0.2em] ${survived ? "text-zinc-500" : "text-red-400"}`}>
          응급 · {survived ? "생존" : "사망"}
        </span>
      </header>
