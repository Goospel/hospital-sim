 ...
 *  30/30/25/20/30이 100/0/0/0/0으로 붕괴하는데 에러는 하나도 안 난다(희망 과·응급 종류와 같은 병). */
 ...
 *
 * 정수인 이유는 부동소수 누적 오차다 — 지분을 소수로 적으면(0.222+0.222+…) 합이 1.0이 아닐 수 있고, 그러면
...
export const EVENT_WEIGHTS: Record<SimEventKind, number> = {
  ...
  NEARBY_CLOSURE: 25,
  /**
   * 20 ← **A3(경증 쏠림) 추가로 합이 커질 때 이것만 절대 기대치를 복원했다.**
   *
   * 가중치는 지분이라 종류가 늘면 기존 넷이 **다 함께** 묽어지는데, 이 하나는 묽어지면 안 된다:
   * B7(방어진료 위축)의 설계 전제가 *"한 번은 경고, 두 번은 구조"*이고 하드락 도달선이 **정확히
   * 소송 2건**이라, 판당 기대치가 2 언저리로 내려오면 그 메시지 장치가 조용히 사라진다.
   * 실측(200시드 × 84일): 15로 두면 기대 3.2 → 2.47건이고 **2건 미만인 판이 17% → 26.5%**로
   * 늘었다. 20이면 지분 14.8%로 A3 이전(15%)에 사실상 복원된다.
   *
   * 나머지 셋을 안 올린 이유: 그것들은 "몇 번 오는가"가 곧 세기라 묽어지는 것이 그대로 튜닝이지만,
   * 소송은 **2라는 문턱이 따로 있는** 유일한 이벤트다. 아래 EVENT_MIX가 지분을 파생하므로 여기
   * 한 줄이 분포·폴백·테스트에 함께 반영된다. 도달 가능성 자체는 `events.test.ts`가 상수들의
   * 산술로 잠근다(가중치 리터럴이 아니라 **파생 불변식**이라 튜닝은 막지 않고 바닥만 지킨다).
   */
  LAWSUIT: 20,
 ...
 * 잦아지는 식으로, 아무도 의도하지 않은 인과가 생기고 상한 표(EVENT_WEIGHTS)의 지분이 거짓이 된다.
