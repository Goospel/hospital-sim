...
// 같은 숫자를 옮기면 부호가 뒤집힌다. 값을 조정할 땐 부호 불변식(I-B1 — `week.test.ts`)을 지킨다.
//
// ⚠️ **고정비는 구현 후 실측으로 한 번 재조정됐다**(계획 표의 2,500/3,000/4,000/5,000 → 아래 값).
// 계획의 값은 "의사 1인 주 처리량"을 크게 잡은 어림이었고, 실제로 돌려 보니 부호가 통째로 뒤집혀
// 있었다 — 미용 의사는 주 41~47건(1,230~1,410만원)뿐이라 2,500이면 적자였고, 반대로 응급을 받는
// 순환기는 응급 수익만으로 주 +10,000 흑자였다. 즉 "옳은 의료를 할수록 장부가 나빠진다"가 정확히
// 거꾸로 서 있었다. 값별 주석에 각 눈금의 실측 근거를 적었다.
// **값의 단일 출처는 이 파일**이다(계획 문서의 표는 그 시작점을 남긴 역사 기록이다).
...
export const SIM_DEPTS: Record<SimDeptKey, SimDeptSpec> = {
  AESTHETICS: {
    ...
    // 800 ← `src/game/types.ts` DEPARTMENTS.AESTHETICS.fixedCostPerDoctorManwon(560)에서 **대소만**
    // 계승했다(모든 과 중 가장 가볍다 — 24시간 대기가 없다). 절대액은 새 스케일로 재조정:
    // 이 시뮬은 하루 8시간·20분 진료라 의사 1인 주 처리량이 옛 층보다 훨씬 작다.
    // ⚠️ 눈금의 근거는 **실측**이다(부호 불변식 조정, 아래 머리말): 미용 의사 1명이 표준 병원
    // 한 주에 보는 외래는 41~47건 = 1,230~1,410만원이라, 흑자가 되려면 주급이 그 아래여야 한다.
    weeklyCostManwon: 800,
  ...
  INTERNAL_MEDICINE: {
    ...
    // 1,500 ← DEPARTMENTS.INTERNAL_MEDICINE(700) — 필수과 중 가장 가볍되 미용보다는 무겁다는
    // 대소를 계승. 옛 주석대로 내과는 24h 대기 비용이 아니라 외래 볼륨 쪽이라 필수과 최저다.
    // 실측 눈금: 내과 의사 1명의 주 외래는 85~89건 = 1,020~1,068만원이라, 이 주급이면 **가장
    // 많이 보는 과가 가장 얇게 진다**(박리다매로도 못 메운다 = 저수가의 형태).
  ...
  GENERAL_SURGERY: {
    ...
    // 4,500 ← DEPARTMENTS.GENERAL_SURGERY(840) — 수술·응급 대기가 붙어 내과보다 무겁다는 대소 계승.
    // 내과와의 간격이 옛 층(840 vs 700)보다 훨씬 벌어진 이유는 **응급 때문**이다: 응급을 받는 과는
    // 처치 수가가 외래의 몇 배라, 주급이 그만큼 무겁지 않으면 응급 몇 건으로 곧바로 흑자가 되어
  ...
  CARDIOLOGY: {
    ...
    examRevenueManwon: 25,
    // 8,000 ← DEPARTMENTS.CARDIOLOGY(1190) — 24시간 심장중재팀 대기가 붙어 무겁다.
    ...
    // 실측 눈금(부호 불변식 I-B1 ⓑ): 순환기 1명 병원의 주 수입 상한은 외래 550~600 + STEMI
