...
/**
 * 합의금 고지 — 의료소송이 **금고에** 남긴 자국을 한 줄로 말한다(`chillNotice`의 짝).
 *
 * 이 줄이 생긴 이유는 12주 실플레이 관측이다: 합의금(800만 × 지역 배율)은 이벤트가 붙는 그
 * 순간 금고에서 빠지는데 **어느 화면에도 액수가 없었다.** 연출문은 "합의금이 청구됐습니다"까지고,
 * 주간 결산표에도 줄이 없어 금고 변화와 표의 합이 안 맞는 것만 남았다.
 *
 * **시점까지 말하는 것이 이 문장의 절반이다** — 액수만 적으면 "그럼 주말에 또 빠지나"가 남는다.
 * 결산표가 이 돈을 총액에 안 넣는 것과 같은 사실이고(week.WeekSummary.lawsuitManwon), 두 화면이
 * 같은 말을 해야 표와 금고가 다시 어긋나 보이지 않는다.
 *
 * **이미 포맷된** 금액 문자열을 받는다 — 접는 정책(억/만원)은 화면 층의 단일 함수가 소유한다
 * (`EpilogueStats.treasuryText`와 같은 계약: 여기서 다시 나누면 단위가 두 벌이 되고, 이 파일이
 * 화면 층을 임포트하면 위상이 뒤집힌다).
 *
 * ⚠️ 보간값 **바로 뒤는 마침표**다(T-094). 「400만원**이**」는 지금 우연히 맞지만(받침 ㄴ), 표기가
 * 「1.2억」으로 접히는 구간이 있고 그 정책은 이 파일 밖에 있다 — 조사를 붙이면 남의 파일을 고치는
 * 날 이 문장이 조용히 틀린다.
 */
export function settlementNotice(costText: string): string {
  return `합의금은 ${costText}. 금고에서 이미 빠져나갔습니다.`
}
