...
//
// 부여는 `pawn.spawnDoctor`가 **nextId 인덱싱**으로 한다 — RNG를 쓰지 않는 것이 계약이다.
// 시드를 소비하면 이름·특성이 도착·응급 스트림과 축을 나눠 갖게 되어, "몇 번째로 뽑았나"가
// 아니라 "그 주에 무슨 일이 있었나"가 사람을 바꾼다.
...
/** 의사 이름 후보 — `nextId % 길이`로 돌아간다(각색 · 실존 인물과 무관).
 *  중복이 없어야 같은 판에서 동명이인이 안 생긴다(테스트가 잠근다). */
...
/** nextId → 이름. 목록을 한 바퀴 돌면 동명이인이 생긴다 — 한 판에 뽑을 수 있는 인원(전국 풀
 *  합계)보다 목록이 길면 실제로는 안 겹친다. */
export function doctorName(nextId: number): string {
  return DOCTOR_NAMES[nextId % DOCTOR_NAMES.length]
}
