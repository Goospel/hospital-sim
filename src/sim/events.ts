 ...
 * (막힌 앞 타일·중복 자리)까지 보므로 결과가 여기보다 적을 수 있다. 그런데도 안전한 이유는
 * 오차의 **방향**이 아니라 **두 수가 만나지 않기 때문**이다: 여기 센 수는 전제 판정(후보에
 * 오르는가)에만 쓰이고, 실제 잠금 K는 `lockedWardBedCount`가 **저쪽 목록 길이**로만 계산한다.
 * 그래서 두 카운트가 아무리 갈려도 K는 언제나 그 목록의 내림 절반이라 「후보가 전부 지워지지는
 * 않는다」가 그대로 성립한다. 갈릴 수 있는 것은 **이벤트가 후보에 오르는가** 하나뿐이고, 그
 * 오차는 "잠금 0인 조용한 하루"로 끝난다.
 *
