...
//
// ⚠️ **2026-07-29에 한 번 다시 박았다** — "의자에 앉는다"(사용자 지시)로 의자가 통행 가능해지고
// 좌석이 의자 **앞**에서 의자 **그 자체**로 옮겨졌다. 규칙이 바뀌었으니 300분의 결과도 바뀌는
// 것이 옳다(전환 무변 계약의 위반이 아니라 **의도한 규칙 변경**이다). 그때의 값 이동은
// `examsDone 3→4 · leftCount 37→36 · revenue 36→48`이고, 원인은 둘이다: ① 의자가 안 막아
// 대기실 안 보행이 짧아졌다 ② 좌석이 겹칠 수 없어져 자리 선택이 달라졌다. 이 각주가 있는
// 이유는 값만 갈아치우면 다음 사람이 "언제 왜 갈렸는지"를 영영 못 찾기 때문이다.
...
describe('영역 전환 회귀 — 300분을 돌린 세계가 전환 전과 같다', () => {
  it('집계·수입·폰 좌표가 전환 전 실측값과 일치한다', () => {
    ...
    expect({
      ...
      examsDone: 4,
      // 대기실 좌석(6석 = 의자 6개)이 차 문간에서 돌아선 인원이 대부분이다 — 좌석 열거가
      // 갈리면 여기가 먼저 운다.
      leftCount: 36,
      revenueManwon: 48,
      // **전부 의자 타일이다** — 대기 환자 6명이 의자 (19,25)·(19,27)·(21,25)·(21,27)·(23,25)·
      // (23,27)에 앉아 있고, (20,15)는 진료 의자에 앉은 환자, (19,16)은 책상 앞의 의사다.
      // 이 배열이 곧 "의자에 앉는다"의 증거다(옛 값은 전부 의자 **앞** 타일이었다).
      pawnTiles: ['19,16', '19,25', '19,27', '20,15', '21,25', '21,27', '23,25', '23,27'],
    })
  })
})
