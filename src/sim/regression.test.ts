...
//
// ⚠️ **2026-07-29에 두 번 다시 박았다.** 값만 갈아치우면 다음 사람이 "언제 왜 갈렸는지"를
// 영영 못 찾으므로 이력을 남긴다. 둘 다 전환 무변 계약의 위반이 아니라 **의도한 규칙 변경**이다.
...
describe('영역 전환 회귀 — 300분을 돌린 세계가 전환 전과 같다', () => {
  it('집계·수입·폰 좌표가 전환 전 실측값과 일치한다', () => {
    ...
    expect({
      ...
      // (19,16)은 책상 앞의 의사, (26,25)·(26,26)은 방금 들어와 걸어오는 중인 환자 둘.
      // 300분 시점에 진료 의자(20,15)가 빈 것도 결과다 — 회전이 빨라져 그 순간 사이가 떴다.
      pawnTiles: ['19,16', '19,25', '19,27', '21,25', '21,27', '23,25', '23,27', '26,25', '26,26'],
    })
  })
})
