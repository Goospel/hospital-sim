...
describe('hardlockReason — 야간 배후진료', () => {
  ...
  it('미용·일반응급은 야간이어도 배후 판정을 받지 않는다 — 배후진료가 필요한 건 필수 응급뿐', () => {
    ...
    expect(hardlockReason(solo, nightGeneral, {}, rosterOf(solo))).toBeNull()
  })
})
...
describe('하드락 vs 선택 — hardlockReason의 null 여부가 곧 disposition', () => {
  ...
  it('일반응급: 응급실 있으면 선택(배후 무관)', () => {
    expect(hardlockReason(collab, general, {}, rosterOf(collab))).toBeNull()
  })
})
