...
//
// ⚠️ **위 두 문단은 옛 층(game/daysim) 시나리오의 실측이다.** 지금 세계(sim)의 눈금은 다르고,
// 그것을 재려면 세 상수가 함께 정하는 **하루 지속가능 부하**를 봐야 한다:
//   FATIGUE_FREE_MIN + (FATIGUE_REST / FATIGUE_PER_OVER_HOUR) × 60
// = 이만큼까지는 그날 오른 피로를 하룻밤 회복이 그대로 되받는다. 밸런스 판정의 기준선이다.
//
// ⚠️ **재튜닝(2026-08-04, FATIGUE_REST 20 → 30)** — 지속가능 부하를 240 → **280**으로 올렸다.
// 실측(`scripts/surgeon-load-probe.ts` · 서울 URBAN 시드 1 · 12주): 20에서는 **응급을 한 건도
// 안 받는 내과 의사의 하루 부하가 L204~276**이라 지속가능선(240) 위였고, 그래서 응급도 1인
// 편성도 아닌 **균형 편성(내과2·미용1)에서조차** 3주말·10주말에 사직이 났다 — 사직이 배치
// 실패의 신호가 아니라 기본 경로였다. 280이면 그 편성이 12주 완주에 사직 0이 되고(실측
// 피로 f11~29 — 감속 지대 34에도 못 미친다), 1인 필수과는 그대로 갈려나간다(외과·순환기
// 1인 모두 1주말 사직). **회복을 올렸지 문턱을 안 올린 이유**: FREE_MIN을 올리면 "몇 건까지가
// 공짜인가"가 통째로 밀려 응급 1건 계약(fatigue.test.ts)까지 다시 잡아야 하는데, 회복은
// **하루를 넘긴 뒤의 복구 속도**만 건드려 하루 안의 대소를 안 흔든다.
//
// ⚠️ **못 산 것 — 응급이 몰린 날은 어떤 회복량으로도 못 막는다**(실측): 외과 2인이어도
// 하루 7건이 오는 날은 각자 L750~1000이라 **그날 안에** 포화한다. 회복은 이튿날 아침에 오지
// 그날 오후에 오지 않아서, 30·35·40은 물론 **60에서도** 외과 2인의 사직이 안 사라졌다
// (오히려 60에서는 인건비를 더 오래 내다 파산하는 판이 생겼다). 이 축의 손잡이는 회복이
// 아니라 응급 도착 분포나 `RESIGN_SATURATED_DAYS`(누적 리셋 없음) 쪽이다.
//
// ⚠️ **옛 층과 공유하는 상수다** — 30은 그 층(`/classic`)의 피로 서사가 살아남는 상한이기도
// 하다: 35 이상이면 옛 층 1인 순환기가 7일을 완주해도 피로 0이 되어 그 층의 사직 기제가
// 통째로 죽는다(session.test.ts 두 계약이 실제로 깨진다). 그래서 30~40 구간에서 **30**이다.
...
export const FATIGUE_REST = 30 // 하룻밤 회복 — 20에서 상향(2026-08-04, 위 재튜닝 절)
 ...
 *
 * ⚠️ **실측 갱신**(2026-08-04 · `scripts/surgeon-load-probe.ts` · 서울 URBAN 시드 1 · 12주 ·
 * FATIGUE_REST 30). 옛 기재("1주차 2일 + 2주차 3일 → 2주차 결산에서 이탈")는 병동·응급이 붙기
 * **전** 옛 층의 곡선이라 지금 세계와 어긋난다. 지금은 훨씬 빠르다:
 *   · 순환기 1인(전부 수용): **1주2일에 포화** → 1주7일에 6일 누적 → **1주말 이탈**.
 *   · 외과 1인도 같다(1주2일 포화 → 1주말 이탈).
 * **응급을 거절하면 사람은 남는다**: 외과 의사의 응급 우선순위를 0으로 두면 12주 사직 0이다 —
 * 대신 응급 수입이 0이라 10주차에 파산한다(실측). 사직과 파산을 맞바꾸는 손잡이지 공짜가 아니다.
 * (회복 상향 전에는 거절해도 3주말에 이탈했다 — 그때는 손잡이가 아예 없었다.)
 */
 ...
 *
 * 포화(FATIGUE_MAX)로 마감한 날만 센다. 회복해도 그 날들은 몸에 남는다는 뜻이고, 균형 편성
 * (내과2·미용1)은 실측상 감속 지대 34에도 못 닿아 이 카운터가 12주 내내 0이다 — 구조적으로
 * 무리한 편성에서만 돌아간다.
 * ⚠️ 이 문장은 **FATIGUE_REST 30 상향(2026-08-04) 덕에 다시 참이 된 것**이다. 회복 20에서는
 * 응급을 한 건도 안 받는 내과 의사조차 2주3일에 포화해 3주말에 이탈했다(실측) — 같은 문장이
 * 그때는 거짓이었다. 곡선 상수를 건드리면 이 문장의 진위가 함께 뒤집히니 프로브로 다시 잰다.
 * 리셋 규칙을 두면 "며칠 쉬게 해 되돌리는" 최적화 표면이 생기는데,
