...
//
// ⚠️ **위 두 문단은 옛 층(game/daysim) 시나리오의 실측이다 — 지금 세계(sim)에서는 "2인이 나누면
// 문턱 아래"가 성립하지 않는다**(실측 2026-08-04 · `scripts/surgeon-load-probe.ts` · 서울 URBAN
// 시드 1). 세 상수가 함께 정하는 **하루 지속가능 부하**는
//   FATIGUE_FREE_MIN + (FATIGUE_REST / FATIGUE_PER_OVER_HOUR) × 60 = 160 + 80 = **240 표준강도분**
// 인데(이만큼까지는 그날 오른 피로를 하룻밤 회복이 되받는다), 실측 하루 부하는 응급을 한 건도
// 안 받는 내과·외과 의사조차 **L197~346**(중앙 ~260)이라 이미 이 선 위다. 응급이 붙는 날은
// L500~850으로 올라간다. 그래서 사직은 배치 실패의 신호가 아니라 **기본 경로**다 —
// 증원해도 진료실이 함께 늘면 처리량만 커져 1인당 부하가 안 줄고(S2), 방을 고정해도
// 응급이 몰린 날(하루 7건)은 두 명이 나눠도 각자 문턱의 두 배를 넘는다(S5).
// 이 비대칭을 고치려면 손잡이는 응급 강도가 아니라 **이 세 상수**(또는 외래 도착량)다.
 ...
 *
 * ⚠️ **실측 갱신**(2026-08-04, `scripts/surgeon-load-probe.ts` · 서울 URBAN 시드 1 · 4주). 옛 기재
 * ("1주차 2일 + 2주차 3일 → 2주차 결산에서 이탈")는 병동·응급이 붙기 **전** 옛 층의 곡선이라
 * 지금 세계와 어긋난다. 지금은 훨씬 빠르다:
 *   · 순환기 1인(전부 수용): **1주2일에 포화** → 1주7일에 6일 누적 → **1주말 이탈**.
 *   · 외과 1인도 같다(1주2일 포화 → 1주말 이탈).
 * 그리고 **응급을 전부 거절해도 막지 못한다**: 외과 의사의 응급 우선순위를 0으로 두면 외래만
 * 남는데도 3주2일에 포화해 3주말에 이탈한다. 거절은 시점을 **미룰** 뿐이다 — 플레이어 선택이
 * 움직인다는 것은 맞지만, 그 선택이 사직을 없애지는 못한다.
 */
 ...
 *
 * 포화(FATIGUE_MAX)로 마감한 날만 센다. 회복해도 그 날들은 몸에 남는다는 뜻이다.
 * ⚠️ **"완편 병원은 실측상 34조차 안 넘어 이 카운터가 영원히 0"이라는 옛 기재는 거짓이다**
 * (실측 2026-08-04, 위 RESIGN_SATURATED_DAYS 주석과 같은 프로브). 서울 URBAN 표준 배치에서는
 * 응급을 한 건도 안 받는 **내과 의사조차** 외래만으로 2주3일에 포화해 3주말에 이탈한다 —
 * 이 카운터는 예외 상황이 아니라 **기본 경로**에서 돈다. 리셋 규칙을 두면
 * "며칠 쉬게 해 되돌리는" 최적화 표면이 생기는데,
