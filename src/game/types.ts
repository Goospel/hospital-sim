...
export type RejectionReason =
  ...
  /**
   * 대기하다 떠났다 — 그 과 의사가 비기를 기다렸으나 한계(PATIENCE_MIN)를 넘겼다.
   *
   * 앞의 넷이 "구조가 **막았다**"라면 이건 "구조가 **늦었다**"다. 병원은 거절한 적이 없고
   * 자리도 결국 났는데, 그게 환자가 버틸 수 있는 시각보다 뒤였을 뿐이다 — 실제 뺑뺑이의
   * 형상이 이쪽에 가깝다. 그래서 별도 태그다(NO_FREE_SPECIALIST로 뭉치면 "의사가 없었다"는
   * 거짓말이 된다 — 있었다, 늦었을 뿐이다).
   */
  | 'LEFT_WAITING'
