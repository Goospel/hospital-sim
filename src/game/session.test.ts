...
describe('7일 루프 — day 전이와 달력 기록', () => {
  ...
  it('DAY_END → advanceDay → 다음 날 RECEIVING(새 큐·자리·델타 리셋)', () => {
    ...
    */
    ...
    expect(d2.receiving!.netProfitDeltaManwon).toBe(0) // 그날 진료 수익은 0에서 시작
  ...
  it('달력 한 칸 = 부문 손익 오늘치 + 그날 진료 수익, 그리고 못 받은 필수 응급을 남긴다', () => {
    ...
    expect(rec.netProfitManwon).toBe(rec.segmentShareManwon + rec.callDeltaManwon)
  ...
  it('weekTotals — 7일치 진료 수익·소송 노출을 합산한다(마지막 날 것만이 아니라)', () => {
    ...
    const sumDelta = s.ledgerDays.reduce((n, r) => n + r.callDeltaManwon, 0)
    const sumLawsuit = s.ledgerDays.reduce((n, r) => n + r.lawsuitExposure, 0)
    expect(totals.netProfitDeltaManwon).toBe(sumDelta)
  ...
  it('부문 손익 오늘치 7일 합 = 주간 전액 — 달력과 결말 장부가 어긋나지 않는다', () => {
    ...
    const sumShares = s.ledgerDays.reduce((n, r) => n + r.segmentShareManwon, 0)
    const weekly = s.hospital!.economics!.segments.reduce((n, x) => n + x.profitManwon, 0)
...
describe('completeWeek 가드 + buildEpilogue', () => {
  ...
  it('양심 경로: 한 주 완주 → 종료 → EPILOGUE, 적자·소송비용·순환기 채용 N', () => {
    // 양심 빌드는 미용 1명뿐이라 워크인 흑자로 순환기 고정비를 못 덮는다. 게다가 필수 응급을 받으려면
    // 선택진료(워크인·배후과 예약)를 거절해 그 과 의사를 비워둬야 해서 유일한 수익원마저 스스로 끊는다.
    // 적자는 '지었기 때문'이 아니라 **고정비 + 원가미달 진료**의 합이고, 이제 장부에 두 줄로 각각 보인다.
    ...
    expect(epi.ledger!.netProfitManwon).toBeLessThan(0) // 적자 + 소송 비용
  })

  it('공범 경로: 채용 0 · 소송비용 없음 · 흑자', () => {
    // ⚠️ 전부 거절(false)로는 이제 흑자가 안 난다 — 부문 손익이 고정비라 진료 0이면 고정비만 남는다.
    // 공범의 흑자는 '필수과를 안 지었다'가 아니라 **워크인을 잔뜩 받았다**에서 온다(수용 정책 true).
    const s = endGame(finishWeek(runWeek(collaborator, true)))
    const epi = buildEpilogue(s)
    expect(epi.ledger!.essentialHires).toBe(0)
    expect(epi.ledger!.netProfitManwon).toBeGreaterThan(0) // 흑자
...
describe('WORLD_EVENT — 외생 이벤트가 위저드 전에 세계를 재구성한다', () => {
  it('enterWorldEvent: LANDING → WORLD_EVENT, 개원 재정중립 패키지가 세계에 확정된다', () => {
    ...
    expect(cardio.fixedCostPerDoctorManwon).toBe(770) // 개혁 본체: 고정비 1,190 → 770
    expect(s.event!.direction).toBe('improve')
  })

  it('enterWorldEvent: 개원 패키지는 산부를 안 건드리고 검사(검진)에서 상쇄한다 (검진 고정비 ↑)', () => {
    const s = enterWorldEvent(startSession())
    const checkup = s.world!.departments.find((d) => d.key === 'CHECKUP')!
    const ob = s.world!.departments.find((d) => d.key === 'OBSTETRICS')!
    expect(checkup.fixedCostPerDoctorManwon).toBe(1_050) // 630 + 420 (재원 상쇄)
    expect(ob.fixedCostPerDoctorManwon).toBe(1_050) // 불변
  })
  ...
  it('beginSetup은 WORLD_EVENT에서도 SETUP으로 가며 world를 보존한다', () => {
    ...
    expect(s.world!.departments.find((d) => d.key === 'CARDIOLOGY')!.fixedCostPerDoctorManwon).toBe(770)
  })

  it('completeSetup은 세계(이벤트 적용본)의 경제를 병원에 반영한다', () => {
    const world = applyEvent(initWorld(), OPENING_EVENT) // 순환기 고정비 770
    const s = completeSetup(conscientious, world)
    const cardioSeg = s.hospital!.economics!.segments.find((seg) => seg.label === '순환기내과')!
    expect(cardioSeg.profitManwon).toBe(-770 * 2) // 기본 세계였다면 −2,380
  })

  it('completeSetup: 개원 패키지 세계에서 검진을 뽑으면 늘어난 고정비가 장부에 반영된다', () => {
    const world = applyEvent(initWorld(), OPENING_EVENT) // 검진 고정비 980
    ...
    expect(seg.profitManwon).toBe(-1_050) // 검사 과보상 인하가 장부에 반영(−1,050 × 1)
  })

  it('completeSetup을 world 없이 부르면 기본 세계 — 기존 흐름 무변경(하위호환)', () => {
    const s = completeSetup(conscientious)
    const cardioSeg = s.hospital!.economics!.segments.find((seg) => seg.label === '순환기내과')!
    expect(cardioSeg.profitManwon).toBe(-1190 * 2) // 기본 −2,380
  })
})
...
describe('주 반복 루프 — WEEK_SUMMARY와 주 경계 전이', () => {
  ...
  it('nextWeek는 세계를 누적한다 — 2주차 이벤트가 1주차 개선 위에 쌓인다', () => {
    ...
    expect(cardio.fixedCostPerDoctorManwon).toBe(770) // 1주차 개선(1,190→770)이 그대로 보존된다
    const thoracic = nw.world!.departments.find((d) => d.key === 'THORACIC_SURGERY')!
    expect(thoracic.fixedCostPerDoctorManwon).toBe(980) // 2주차 흉부외과 가산(1,400→980)이 그 위에 쌓인다
  })
...
describe('성장 상태 — 개원 보존 + 금고', () => {
  ...
  it('completeSetup이 choices·beds·treasury·system을 보존한다', () => {
    ...
    expect(s.treasury).toBe(SETUP_BUDGET_MANWON - 35_000) // 개원 잔액
...
describe('GROWTH — 재투자 적용', () => {
  ...
  function grown() {
    ...
    s = { ...s, treasury: 100_000, phase: 'GROWTH', week: 2 }
    return s
  }
  ...
  it('성장 비용 = 채용 증분 + 병상 증설', () => {
    ...
    expect(growthCostOf(s, next, 5)).toBe(15_000 + 10_000) // 순환기 1명(15,000) + 병상 3→5(10,000)
  })
  ...
  it('applyGrowth: 병원 재구성 + 금고·풀 차감 + choices 갱신', () => {
    ...
    expect(after.treasury).toBe(100_000 - 25_000)
  ...
  it('금고 초과 성장 불가', () => {
    const s = { ...grown(), treasury: 10_000 }
    const next = { hospitalName: '한바다', doctors: { AESTHETICS: 1, CARDIOLOGY: 2 } } // 15,000 필요
    expect(canApplyGrowth(s, next, 3)).toBe(false)
  })
  ...
  it('world 미정의여도 해고 방지 가드가 유지된다 (deps 폴백 = DEPARTMENTS)', () => {
    ...
    const s = { ...base, phase: 'GROWTH' as const, treasury: 100_000, world: undefined }
...
/*
  ...
  ## 2026-07-23 수익 구조 재설계로 이 블록의 무게중심이 옮겨갔다

  옛 세 번째 테스트는 *"콜 기여분 < 구조 기여분"*이었다. 그건 부문 손익이 **수익**이던 시절의 명제고,
  고정비로 뒤집힌 지금은 **참이면 안 된다** — 구조는 이제 −만 만들고, 흑자는 콜에서만 온다.
  그래서 그 자리를 실제로 이 재설계를 정의하는 두 불변식이 대신한다:

    [I8-a] 진료를 안 보면 흑자가 날 수 없다 — "전부 거절해도 주 303억"이 이 작업의 출발점이었다.
    [I8-b] 부문 손익은 항상 ≤ 0 — 수익이 두 층에서 세지는 이중 계상(T-069)의 구조적 재발 방지.

  스케일 상한(I8) 자체는 그대로 남긴다. 이제 그걸 위협하는 건 부문 손익이 아니라 **콜 볼륨 × 마진**이다.
*/
describe('[I8] 주간 순이익 스케일 — 콜 볼륨이 늘어도 예산 스케일을 벗어나지 않는다', () => {
  const CAP = 4 * SETUP_BUDGET_MANWON
  ...
  const weekNet = (choices: SetupChoices, accept: Policy) =>
    runWeek(choices, accept).ledgerDays.reduce((n, d) => n + d.netProfitManwon, 0)
  ...
  it('[I8-a] 전부 거절하면 반드시 적자 — 진료 0인데 흑자가 나던 구조를 막는다', () => {
    // 옛 모델에선 미용 3명을 뽑아 놓기만 하면 콜을 한 통도 안 받고 주 303억이 났다.
    // 이제 남는 건 고정비뿐이다. 수익과를 꽉 채운 빌드조차 그렇다.
    expect(weekNet(maxRevenue, false)).toBeLessThan(0)
    expect(weekNet(collaborator, false)).toBeLessThan(0)
  })

  it('[I8-b] 부문 손익은 어떤 빌드에서도 ≤ 0 — 이 층엔 수익이 없다(이중 계상 구조적 차단)', () => {
    for (const choices of [collaborator, conscientious, maxRevenue]) {
      const end = runWeek(choices, true)
      for (const seg of end.hospital!.economics!.segments) expect(seg.profitManwon).toBeLessThan(0)
      expect(end.ledgerDays.reduce((n, d) => n + d.segmentShareManwon, 0)).toBeLessThan(0)
    }
  })

  it('[I8-c] 흑자는 진료에서만 온다 — 콜 기여분이 고정비를 덮을 때에만 주간 순이익 > 0', () => {
    const structureOnly = weekNet(maxRevenue, false) // 고정비만(음수)
