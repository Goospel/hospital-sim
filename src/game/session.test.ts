...
import {
  ...
  beginEmergency, completeWeek, nextWeek, beginWeek, endGame, survivedEmergency,
...
/** day-1 RECEIVING 상태에서 7일을 통째로 흘려 INTERSTITIAL까지 간다(각 날 마감 + 다음 날 전진). */
...
/** 개원(위저드 완료)부터 한 주를 흘려 INTERSTITIAL 직전까지 간다. */
...
/** 한 주 완주: INTERSTITIAL → 응급(완료) → WEEK_SUMMARY. TRANSFER는 출구 h6로 수용해 끝낸다. */
function finishWeek(interstitial: SessionState): SessionState {
  let s = beginEmergency(interstitial)
  const em = s.emergency!
  if (em.mode === 'TRANSFER') {
    s = { ...s, emergency: { mode: 'TRANSFER', game: attemptTransfer(em.game, 'h6', 12) } }
  }
  return completeWeek(s)
}
...
describe('completeWeek 가드 + buildEpilogue', () => {
  it('TRANSFER가 진행중이면 completeWeek 에러', () => {
    const s = beginEmergency(runWeek(collaborator))
    expect(() => completeWeek(s)).toThrow()
  })

  it('양심 경로: 한 주 완주 → 종료 → EPILOGUE, 생존·적자·소송비용·순환기 채용 N', () => {
...
describe('WORLD_EVENT — 외생 이벤트가 위저드 전에 세계를 재구성한다', () => {
  ...
  it('enterWorldEvent: 개원 재정중립 패키지가 산부도 상쇄한다 (산부 -16)', () => {
    const s = enterWorldEvent(startSession())
    const ob = s.world!.departments.find((d) => d.key === 'OBSTETRICS')!
    expect(ob.profitPerDoctorBillions).toBe(-16) // 재정중립 상쇄(-10 - 6)
  })
  ...
  it('completeSetup: 개원 패키지 세계에서 산부를 뽑으면 economics에 -16이 반영된다', () => {
    const world = applyEvent(initWorld(), OPENING_EVENT) // 산부 -16
    const s = completeSetup({ hospitalName: '양심병원', doctors: { OBSTETRICS: 1 } }, world)
    const obSeg = s.hospital!.economics!.segments.find((seg) => seg.label === '산부인과')!
    expect(obSeg.profitBillions).toBe(-16) // 재정중립 상쇄가 장부에 반영(-16 * 1)
  })
...
// EMERGENCY → completeWeek → WEEK_SUMMARY →(nextWeek: 새 세계 이벤트, 같은 병원) 또는 (endGame: EPILOGUE).
// 세계 변화는 주마다 누적되고, 에필로그는 그때까지의 모든 주를 합산한다.
describe('주 반복 루프 — WEEK_SUMMARY와 주 경계 전이', () => {
  it('completeWeek: 응급(완료) → WEEK_SUMMARY, 이번 주가 history에 누적된다', () => {
  ...
  it('응급이 아니면 completeWeek 에러(가드)', () => {
    expect(() => completeWeek(runWeek(collaborator))).toThrow() // INTERSTITIAL에서 호출
  })
  ...
  it('nextWeek는 세계를 누적한다 — 2주차 악화가 1주차 개선 위에 쌓인다', () => {
    ...
    expect(nw.event!.direction).toBe('worsen') // week2 = selectEvent(1) = 악화
    const cardio = nw.world!.departments.find((d) => d.key === 'CARDIOLOGY')!
    expect(cardio.profitPerDoctorBillions).toBe(-6) // 1주차 개선(-12→-6)이 보존된다
    expect(cardio.hireCostBillions).toBe(36) // 2주차 악화(30→36)가 그 위에 쌓인다
  })
  ...
  it('WEEK_SUMMARY가 아니면 nextWeek·endGame 에러(가드)', () => {
    const inter = runWeek(collaborator)
    expect(() => nextWeek(inter)).toThrow()
    expect(() => endGame(inter)).toThrow()
  })
  ...
  it('survivedEmergency: IN_HOUSE는 생존, TRANSFER는 ACCEPTED만 생존', () => {
    expect(survivedEmergency(beginEmergency(runWeek(conscientious)).emergency!)).toBe(true)
    expect(survivedEmergency(beginEmergency(runWeek(collaborator)).emergency!)).toBe(false) // IN_PROGRESS
  })
