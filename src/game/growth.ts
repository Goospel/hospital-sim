...
import { DEPARTMENTS, SETUP_BUDGET_MANWON, setupHiringCostManwon } from './setup'
...
export function initialTreasury(choices: SetupChoices, departments: DepartmentSpec[] = DEPARTMENTS): number {
  return SETUP_BUDGET_MANWON - setupHiringCostManwon(choices, departments)
}
...
export function doctorDeltaCost(
  ...
  return departments.reduce((sum, d) => {
    ...
    return sum + (delta > 0 ? delta * d.hireCostManwon : 0)
  }, 0)
}
