...
# ── 케이스 11: 점(.)으로 시작하는 대상 → 거부 (T-063) ──────────────────────
# 옵시디언은 점 폴더·점 파일을 볼트에서 **아예 제외**한다. 그래서 디스크에 파일이
# 있어도(규칙 3 통과) 영원히 해석되지 않아 그래프에 유령 노드로 남는다. 게다가
# 상대경로 형태가 다르면(./ ../ ../../) 같은 파일이 **여러 노드로 갈린다**.
# 대상을 **실제로 만든다** — 규칙 3(존재)이 통과해야 규칙 4만 남아 진짜 Red가 된다.
# 실제 저장소에서도 deploy-pages.yml 은 디스크에 있는데 옵시디언에선 유령이었다.
New-Item -ItemType Directory -Path (Join-Path $fixtureRoot '.github/workflows') -Force | Out-Null
[IO.File]::WriteAllText((Join-Path $fixtureRoot '.github/workflows/x.yml'), 'on: push')
[IO.File]::WriteAllText((Join-Path $fixtureRoot '.env.example'), 'KEY=')

$dotDir  = New-Md 'n.md' "---`ntags:`n  - type/meta`n---`n`n[워크플로](.github/workflows/x.yml)`n"
$dotFile = New-Md 'o.md' "---`ntags:`n  - type/meta`n---`n`n[env](.env.example)`n"
$relOk   = New-Md 'p.md' "---`ntags:`n  - type/meta`n---`n`n[상대](./a.md) 과 [부모](../cdt/a.md)`n"

Write-Host '케이스 11: 점으로 시작하는 경로 조각을 가리키는 링크는 거부한다'
$r = Invoke-Check @($dotDir)
Assert-That ($r.ExitCode -ne 0) '점 폴더(.github/) 대상이면 exit != 0' "실제 exit=$($r.ExitCode)`n$($r.Output)"
Assert-That ($r.Output -match 'LINKS-CHECK:\s*DOTPATH') '점 경로 전용 ASCII 마커' $r.Output
$r = Invoke-Check @($dotFile)
Assert-That ($r.ExitCode -ne 0) '점 파일(.env.example) 대상이면 exit != 0' "실제 exit=$($r.ExitCode)`n$($r.Output)"

Write-Host '케이스 12: ./ 와 ../ 는 점 경로가 아니다(오탐 방지)'
$r = Invoke-Check @($relOk)
Assert-That ($r.ExitCode -eq 0) './ · ../ 상대경로는 exit 0' "실제 exit=$($r.ExitCode)`n$($r.Output)"
